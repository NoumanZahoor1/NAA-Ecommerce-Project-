...
import { FaBars, FaTimes, FaHome, FaSignOutAlt, FaChartBar, FaBox, FaClipboardList, FaUsers, FaCog } from 'react-icons/fa';
...
const AdminLayout = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-950 transition-colors duration-300 overflow-hidden">
      ...
      {/* Responsive Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 text-white p-6 flex flex-col border-r border-gray-800 
        transition-transform duration-500 ease-in-out md:translate-x-0 md:static
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="flex justify-between items-center mb-10">
          <h1 className="text-2xl font-black tracking-tight flex items-center gap-2">
            <span className="text-red-600">NAA</span> ADMIN
          </h1>
          <button onClick={() => setIsSidebarOpen(false)} className="md:hidden p-2 text-white/60">
            <FaTimes size={20} />
          </button>
        </div>
        ...
        <nav className="flex flex-col gap-2 flex-1">
          {[
            { name: 'Dashboard', path: '/admin/dashboard', icon: <FaChartBar size={16} /> },
            { name: 'Products', path: '/admin/products', icon: <FaBox size={16} /> },
            { name: 'Orders', path: '/admin/orders', icon: <FaClipboardList size={16} /> },
            { name: 'Customers', path: '/admin/customers', icon: <FaUsers size={16} /> },
            { name: 'Settings', path: '/admin/settings', icon: <FaCog size={16} /> },
          ].map((item) => (
            <Link 
              key={item.name} 
              to={item.path} 
              className={`
                flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-300 text-sm font-bold uppercase tracking-wider
        ...
        <div className="mt-auto space-y-4 border-t border-white/10 pt-6">
          <button
            ...
            className="flex items-center gap-3 w-full px-4 text-sm text-red-400 hover:text-red-300 transition-colors font-black uppercase tracking-wider"
          >
            <FaSignOutAlt size={16} />
            Logout
          </button>
          <Link to="/" className="flex items-center gap-3 px-4 text-sm text-white/40 hover:text-white transition-colors font-medium uppercase tracking-widest text-[10px]">
            <FaHome size={16} />
            Back to Store
          </Link>
        </div>
      ...
      {/* Main Content Area */}
      <div className="relative flex-1 flex flex-col min-w-0 h-screen overflow-hidden bg-[#FAF9F6] dark:bg-black/20">
        {/* Mobile Header Toggle */}
        <header className="flex md:hidden items-center justify-between px-6 py-4 bg-white dark:bg-black border-b border-gray-200 dark:border-white/10 shrink-0">
          <button onClick={() => setIsSidebarOpen(true)} className="p-2 text-black dark:text-white">
            <FaBars size={24} />
          </button>
          <span className="font-black tracking-tighter uppercase dark:text-white">NAA Admin</span>
          <div className="w-10" /> {/* Spacer */}
        </header>

        <main className="flex-1 p-4 md:p-10 overflow-y-auto dark:text-gray-100">
