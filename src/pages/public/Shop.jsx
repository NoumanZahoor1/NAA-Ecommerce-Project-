...
import { getProductVibe, parseNaturalLanguageQuery } from '../../utils/searchUtils';
    ...
    const filteredProducts = products.filter(product => {
        ...
        if (filters.searchQuery) {
            ...
            if (parsed.keywords.length > 0) {
                ...
                // If it doesn't match any keyword AND doesn't match any vibe, reject it.
                const hasKeyword = parsed.keywords.some(kw => searchText.includes(kw));
                // Wait, if vibe matched this product, let it pass even if keyword doesn't match exactly!
                const productVibes = getProductVibe(product);
                const hasVibe = parsed.vibes.length > 0 && parsed.vibes.some(v => productVibes.includes(v));
                
                if (!hasKeyword && !hasVibe) {
                    return false;
                }
            }
        } else if (filters.vibe) {
            const productVibes = getProductVibe(product);
            if (!productVibes.includes(filters.vibe)) return false;
        }
