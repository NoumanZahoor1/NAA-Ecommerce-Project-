...
const VisualSearchModal = ({ isOpen, onClose }) => {
    ...
    const findMatchingProducts = (allProducts, type, colors) => {
        ...
        const typeAliases = {
            'jacket': ['jacket', 'blazer', 'coat', 'suit', 'outerwear', 'bomber', 'puff', 'tuxedo', 'leather', 'windbreaker', 'parka'],
            't-shirt': ['t-shirt', 'tee', 'tshirt', 'polo'],
            'shirt': ['shirt', 'blouse', 'button', 'polo'],
            'pants': ['pants', 'trousers', 'jeans', 'chinos', 'denim', 'bottoms'],
            'hoodie': ['hoodie', 'sweatshirt', 'pullover'],
            'sweater': ['sweater', 'cardigan', 'knit'],
            'dress': ['dress', 'gown', 'skirt', 'jumpsuit'],
            'shoes': ['shoes', 'sneakers', 'boots', 'sandals', 'footwear'],
            'bag': ['bag', 'backpack', 'purse', 'tote', 'wallet']
        };

        const typeLower = type.toLowerCase();
        const aliases = typeAliases[typeLower] || [typeLower];
        ...
        const scored = allProducts.map(product => {
            ...
            if (aliases.some(alias => name.includes(alias) || category.includes(alias))) {
                score += 60;
                hasTypeMatch = true;
            } else if (name.includes(typeLower) || category.includes(typeLower)) {
                score += 50;
                hasTypeMatch = true;
            }

            // ❌ STRICT FILTER: Only show items that match the detected type or its aliases
            if (!hasTypeMatch) return null;

            // Color matching (30 points max)
            if (colors.length > 0 && productColors.length > 0) {
                const primaryMatch = productColors.some(c => c.toLowerCase() === colors[0].toLowerCase());
                if (primaryMatch) score += 20;

                const secondaryMatches = colors.slice(1).filter(c => 
        ...
        // Filter and sort
        return scored
            .filter(p => p !== null && p.matchScore > 0)
