...
const Navbar = () => {
    ...
    const handleVoiceSearch = () => {
        ...
        recognition.onresult = (event) => {
            ...
            // Advanced match scoring to find the absolute best matched product
            const transcriptLower = transcript.toLowerCase();
            const words = transcriptLower.split(/\s+/).filter(w => w.length > 2); // Ignore short words
            ...
            const scoredProducts = products.map(p => {
                ...
                if (pText.includes(transcriptLower)) score += 50;
                if (p.name.toLowerCase() === transcriptLower) score += 100; // Perfect exact name match
                
                // Word matches
                words.forEach(word => {
                    if (p.name.toLowerCase().includes(word)) score += 10;
                    else if (pText.includes(word)) score += 3;
                });
            ...
            // ONLY auto-open if the score is extremely high (e.g., perfect phrase match like "leather jacket")
            if (scoredProducts.length > 0 && scoredProducts[0].score >= 50) {
                const bestMatch = scoredProducts[0].product;
                speakResponse(`Opening the ${bestMatch.name} for you.`);
                navigate(`/product/${bestMatch.id}`);
            } else {
                // Otherwise fallback to basic shop search (good for conversational queries like "do you have red dress")
                speakResponse(`Searching for ${transcript}.`);
                const parsedFilters = parseNaturalLanguageQuery(transcript);
                const params = new URLSearchParams();
                if (parsedFilters.colors.length > 0) params.set('colors', parsedFilters.colors.join(','));
                if (parsedFilters.categories.length > 0) params.set('category', parsedFilters.categories[0]);
                if (parsedFilters.maxPrice) params.set('maxPrice', parsedFilters.maxPrice);
                ...
                <div className="max-w-7xl mx-auto px-6">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="hidden md:flex items-center gap-8">
                            {navLinks.map((link) => (
                                ...
                                >
                                    ...
                                    <span className={`absolute bottom-0 left-0 w-full h-[1px] transform -translate-x-full group-hover:translate-x-0 transition-transform duration-300 ${link.isSale || link.isAdmin ? 'bg-red-600 dark:bg-red-500' : 'bg-black dark:bg-white'
                                        }`} />
                                </Link>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            {user ? (
                                <Link to={isAdmin ? "/admin/dashboard" : "/dashboard"} className="text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white transition-colors" >
                                    <FaUser size={18} />
                                </Link>
                            ) : (
                                <Link to="/login" className="text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white transition-colors">
                                    <FaUser size={18} />
                                </Link>
                            )}
                                ...
                                {navLinks.map((link) => (
                                    <div key={link.name} className="overflow-hidden">
                                        <motion.div variants={linkVars}>
                                            <Link
                                                to={link.path}
                                                className={`text-5xl font-black uppercase tracking-tighter hover:text-gray-400 transition-colors ${link.isAdmin ? 'text-red-600' : ''}`}
