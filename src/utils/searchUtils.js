...
const VIBE_KEYWORDS = {
    ...
    formal: ['formal', 'professional', 'business', 'office', 'smart', 'wedding', 'suit', 'tuxedo'],
    casual: ['casual', 'everyday', 'relaxed', 'comfortable'],
    athletic: ['athletic', 'sport', 'gym', 'active', 'performance', 'workout', 'training', 'running'],
    ...
    elegant: ['elegant', 'sophisticated', 'classy', 'refined', 'chic', 'party', 'gala', 'evening']
};
...
export const filterProductsByNLQuery = (products, filters) => {
    return products.filter(product => {
        ...
        // Vibe/Occasion filter
        if (filters.vibes.length > 0) {
            const productVibes = getProductVibe(product);
            const hasVibe = filters.vibes.some(v => productVibes.includes(v));
